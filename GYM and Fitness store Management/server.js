...
const requireClientAuth = (req, res, next) => {
  if (req.session.userId && req.session.userType === 'client') {
    ...
    // Log for debugging:
    console.warn(`Attempted access to protected client route without auth. Session:`, req.session);
    res.status(403).json({ success: false, message: 'Access denied. Client authentication required.' });
  }
};
...
app.get('/api/products', requireClientAuth, async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('SELECT ProductID, Name, Description, Category, Brand, Price, Stock, Image FROM products WHERE IsActive = 1 AND Stock > 0');
    ...
    const products = rows.map(product => {
      if (product.Image) {
        ...
        const imageBase64 = `data:image/webp;base64,${product.Image.toString('base64')}`;
        return { ...product, Image: imageBase64, Price: parseFloat(product.Price) }; // Convert Price to float here
      }
      console.log(`DEBUG: Product ${product.Name} Price type after conversion: ${typeof parseFloat(product.Price)}, value: ${parseFloat(product.Price)}`);
...
app.post('/api/cart/add', requireClientAuth, async (req, res) => {
  ...
  if (!productId || !quantity || quantity <= 0) {
    console.warn(`Invalid input for adding to cart: ProductID ${productId}, Quantity ${quantity}`);
    return res.status(400).json({ success: false, message: 'Invalid product ID or quantity.' });
  }

  try {
    console.log(`Client ${clientId} attempting to add ProductID ${productId} with quantity ${quantity} to cart.`);
    // Fetch product details to ensure it's valid and get current stock/price
    const [productRows] = await pool.query('SELECT ProductID, Name, Price, Stock FROM products WHERE ProductID = ? AND IsActive = 1', [productId]);
    ...
    console.log('DEBUG: Cart before map in add:', typeof req.session.cart, req.session.cart);
    ...
    req.session.cart = req.session.cart.map(item => {
      if (item.productId === productId) {
        ...
        if (product.Stock < newQuantity) {
            ...
            console.warn(`Cannot add more of ProductID ${productId}. Total in cart would exceed stock. Current in cart: ${item.quantity}, Adding: ${quantity}, Available: ${product.Stock}`);
    ...
    if (!itemAdded) {
      ...
      if (product.Stock < quantity) {
        console.warn(`Insufficient stock for ProductID ${productId}. Requested: ${quantity}, Available: ${product.Stock}`);
    ...
    if (itemAdded) {
      console.log(`ProductID ${productId} added to cart. Current cart:`, req.session.cart);
      res.json({ success: true, message: 'Product added to cart successfully!', cart: req.session.cart });
    } else {
        // This path is hit if product was already in cart and attempted add exceeded stock.
        res.status(400).json({ success: false, message: `Could not add ProductID ${productId} to cart. Total quantity would exceed available stock (${product.Stock}).` });
    }

  } catch (err) {
    console.error('Error adding product to cart:', err);
    res.status(500).json({ success: false, message: 'Failed to add product to cart', error: err.message });
  }
});
...
app.post('/api/cart/remove', requireClientAuth, (req, res) => {
  ...
  if (!req.session.cart) {
    console.log(`Client ${clientId} attempted to remove ProductID ${productId}, but cart is empty.`);
    return res.status(404).json({ success: false, message: 'Cart is empty.' });
  }

  // DEBUG: Log cart state before length access and filter
  console.log('DEBUG: Cart before length access and filter in remove:', typeof req.session.cart, req.session.cart);

  const initialCartLength = req.session.cart.length;
  req.session.cart = req.session.cart.filter(item => item.productId !== productId);

  if (req.session.cart.length < initialCartLength) {
    console.log(`ProductID ${productId} removed from cart. Current cart:`, req.session.cart);
    res.json({ success: true, message: 'Product removed from cart.', cart: req.session.cart });
  } else {
    console.warn(`Client ${clientId} attempted to remove non-existent ProductID ${productId} from cart.`);
...
app.post('/api/purchase', requireClientAuth, async (req, res) => {
  ...
  if (cart.length === 0) {
    console.warn(`Client ${clientId} attempted to purchase with an empty cart.`);
    return res.status(400).json({ success: false, message: 'Your cart is empty. Please add products before purchasing.' });
  }
  ...
  try {
    ...
    for (const item of cart) {
      const [productRows] = await connection.query('SELECT Stock, Price FROM products WHERE ProductID = ? AND IsActive = 1', [item.productId]);

      if (productRows.length === 0) {
        throw new Error(`Product ${item.name} (ID: ${item.productId}) not found or inactive.`);
      }

      const { Stock: currentStock, Price: unitPrice } = productRows[0];

      if (currentStock < item.quantity) {
        throw new Error(`Insufficient stock for ${item.name}. Only ${currentStock} available, but ${item.quantity} requested.`);
      }

      totalAmount += item.quantity * parseFloat(unitPrice);
      productUpdates.push({ productId: item.productId, newStock: currentStock - item.quantity });
    }

    // 2. Create Order
    const orderDate = dayjs().utcOffset(BD_OFFSET).format('YYYY-MM-DD HH:mm:ss');
    ...
    console.log(`Order ${orderId} created for client ${clientId} with total amount ${totalAmount.toFixed(2)}.`);

    // 3. Create Order Items
    for (const item of cart) {
      // Re-fetch price from DB to ensure consistency and prevent client-side tampering
      const [productPriceRows] = await connection.query('SELECT Price FROM products WHERE ProductID = ?', [item.productId]);
    ...
    console.log(`Payment ${paymentId} recorded for OrderID ${orderId} using method ${paymentMethod}.`);

    // 5. Update Order with PaymentID
    await connection.query('UPDATE orders SET PaymentID = ? WHERE OrderID = ?', [paymentId, orderId]);
    console.log(`Order ${orderId} updated with PaymentID ${paymentId}.`);


    // 6. Update Product Stock
    for (const update of productUpdates) {
      await connection.query('UPDATE products SET Stock = ? WHERE ProductID = ?', [update.newStock, update.productId]);
    }
    ...
    res.json({ success: true, message: 'Purchase completed successfully!', orderId: orderId, totalAmount: totalAmount.toFixed(2) });
